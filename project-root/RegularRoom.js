class RegularRoom extends Phaser.Scene {
    ...
    create() {
        ...
        switch (roomType) {
            case 0:
                ...
                break;
            case 1:
                this.createSecondRoomLayout(roomCenterX, roomCenterY, roomWidth, roomHeight);
                break;
            case 2:
                this.createThirdRoomLayout(roomCenterX, roomCenterY, roomWidth, roomHeight);
                break;
        }
    ...
    createLayoutOne(centerX, centerY, roomWidth, roomHeight) {
        ...
        for (let y = centerY - roomHeight / 2; y <= centerY + roomHeight / 2; y += wallTileSize) {
            this.walls.create(centerX - roomWidth / 2, y, 'wall').setDisplaySize(wallTileSize, wallTileSize).setSize(wallTileSize, wallTileSize).setOrigin(1, 0.5).refreshBody();
            this.walls.create(centerX + roomWidth / 2, y, 'wall').setDisplaySize(wallTileSize, wallTileSize).setSize(wallTileSize, wallTileSize).setOrigin(0, 0.5).refreshBody();
        }

        for (let x = centerX - roomWidth / 2; x <= centerX + roomWidth / 2; x += wallTileSize) {
            this.walls.create(x, centerY - roomHeight / 2, 'wall').setDisplaySize(wallTileSize, wallTileSize).setOrigin(0.5, 1).refreshBody();
            this.walls.create(x, centerY + roomHeight / 2, 'wall').setDisplaySize(wallTileSize, wallTileSize).setOrigin(0.5, 0).refreshBody();
        }
        ...
        holes.forEach(hole => {
            graphics.fillRect(hole.x - hole.size / 2, hole.y - hole.size / 2, hole.size, hole.size);
        });
        graphics.setDepth(0);
    }
    ...
    createThirdRoomLayout(centerX, centerY, roomWidth, roomHeight) {
        ...
        obstacles.forEach(obstacle => {
            graphics.fillRect(obstacle.x - obstacle.size / 2, obstacle.y - obstacle.size / 2, obstacle.size, obstacle.size);
        });
        graphics.setDepth(0);
    }
